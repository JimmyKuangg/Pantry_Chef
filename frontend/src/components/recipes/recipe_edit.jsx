...
export default class RecipeEditForm extends Component {
    ...
    for (let j = 0; j < propsIngredients.length; j++) {
      for (let k = 0; k < hashCheck.length; k++) {
        if (propsIngredients[j][0] === hashCheck[k]['ingredient']) {
          newIngredients.push({
            _id: propsIngredients[j][1],
            ingredient: propsIngredients[j][0],
  ...
  backToName() {
    for (let i = 0; i < this.state.ingredients.length; i++) {
      if (this.state.ingredients[i]['_id'] === undefined) {
        for (let j = 0; j < this.props.ingredients; j++) {
          if (
            this.state.ingredients[i]['ingredient'] ===
            this.props.ingredients[j][0]
          ) {
    ...
    for (let i = 0; i < this.state.categories.length; i++) {
      if (this.state.categories[i]['_id'] === undefined) {
        for (let j = 0; j < this.props.categories.length; j++) {
          if (this.state.categories[i] === this.props.categories[j]._id) {
            console.log(this.state.categories);
            this.setState({
              [this.state.categories[i]]: this.props.categories[j].name,
            });
          }
        }
      }
